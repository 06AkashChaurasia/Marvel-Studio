...
// GET API Endpoint for bookings
app.get('/api/bookings', async (req, res) => {
    try {
        const bookings = await Booking.find(); // Fetch all bookings from MongoDB
        res.status(200).json(bookings); // Send the bookings data as JSON
    } catch (error) {
...
app.get('/api/enquiries', async (req, res) => {
  try {
    const enquiries = await Enquiry.find(); // Fetch all enquiries from MongoDB
    res.json(enquiries); // Send the data to the frontend
  } catch (error) {
